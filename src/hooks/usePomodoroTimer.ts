...
export function usePomodoroTimer(initialConfig: Partial<TimerConfig> = {}): UsePomodoroTimerReturn {
  const configRef = useRef<TimerConfig>({ ...DEFAULT_CONFIG, ...initialConfig })
  const statsRef = useRef<PersistedStats>(loadStats())
  const phaseRef = useRef<TimerPhase>('focus')
  const timeLeftRef = useRef<number>(getDuration('focus', configRef.current))
  const totalTimeRef = useRef<number>(getDuration('focus', configRef.current))
  const isRunningRef = useRef<boolean>(false)
  const sessionCountRef = useRef<number>(0)
  const cyclesRef = useRef<number>(0)
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null)

  const [phase, setPhase] = useState<TimerPhase>(phaseRef.current)
  const [timeLeft, setTimeLeft] = useState<number>(timeLeftRef.current)
  const [totalTime, setTotalTime] = useState<number>(totalTimeRef.current)
  const [isRunning, setIsRunning] = useState<boolean>(isRunningRef.current)
  const [sessionCount, setSessionCount] = useState<number>(sessionCountRef.current)
  const [cycles, setCycles] = useState<number>(cyclesRef.current)
  const [totalFocusMinutes, setTotalFocusMinutes] = useState<number>(statsRef.current.totalFocusMinutes)
  const [todayFocusMinutes, setTodayFocusMinutes] = useState<number>(statsRef.current.todayFocusMinutes)
  const [weekFocusMinutes, setWeekFocusMinutes] = useState<number>(statsRef.current.weekFocusMinutes)
